...
bounds = {
    k: ([0.01, 0.05] if "ads" in k else [0.5, 2.0] if "des" in k else [50, 150])
    for k in param_config
}
sur = Surrogate(
    proc, param_config, bounds, metrics=["retention_time"], n_train=128, seed=0
)
sur.train()
...
print(
    f"Breusch–Pagan p-value: {bp_p:.3f} "
